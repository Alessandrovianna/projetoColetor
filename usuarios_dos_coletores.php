    ...
    <body class="body-relatorios">
        ...
        <table class="tabela-usuarios" border="0" width="80%">
            ...
            if(isset($_POST['buscar'])){
                ...
                $sql = "SELECT * FROM usuarios_dos_coletores WHERE status = 'Ativado' ORDER BY data_cadastro";
            }
            $sql = $pdo->query($sql);
            if($sql->rowCount() > 0) {
                foreach($sql->fetchAll() as $usuarios) {
                    ...
                    ?>
                    <td class="td-button"><a class="button"  onclick="return confirm('Deseja desativar o usuário?');" href="excluir_usuario.php?id_user=<?=$usuarios['id_user']?>">Desativar</a></td>
                    <?php
                    echo '</tr>';
                }     
            }
...
<!--<td class="td-usuarios"><a class="button" onclick="return confirm('Deseja excluir o usuario ?');" href="remover_usuario.php?id=< //?php$usuarios['id_user'];?>">Remover</a>;-->
